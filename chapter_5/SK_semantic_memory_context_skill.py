...
You are an expert at extracting relevant context from a user input.
Your goal is to extract the most relevant questions you can use to ask memory to provide additional context about a query.
Extract 5 relevant questions from the user input in a comma seperated list of questions.
[INPUT]
{{$input}}
[END INPUT]                                                
                                                 """)
...
async def update_memory_context(history, input):    
    ...
    for i, question in enumerate(questions.result.split(",")):
        context[f"preference{i+1}"] = question
    context[sk.core_skills.TextMemorySkill.COLLECTION_PARAM] = "preferences"
    context[sk.core_skills.TextMemorySkill.RELEVANCE_PARAM] = "0.8"
...
async def chat(kernel: sk.Kernel,
    ...
    context = await update_memory_context(context["chat_history"], user_input)
