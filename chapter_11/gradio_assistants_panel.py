...
def assistants_panel():
    ...
    def get_assistant_details(assistant_key):
        if assistant_key == 'new':
            ...
            assistant = api.retrieve_assistant(assistant_options[assistant_key])
            tools = get_tools(assistant.tools)
            files = []
            return assistant.name, assistant.id, assistant.instructions, assistant.model, tools, files, assistant.response_format, assistant.temperature, assistant.top_p
    ...
    for control in controls:
        control.change(
            fn=api.update_assistant,
            inputs=controls
        )

    add_button.click(
        api.create_assistant,         
        inputs=[assistant_name_new,
